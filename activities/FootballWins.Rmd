...
ANSWER: Yes, it is a useful model. It shows everything you need to know from these sorts of tests. It helps us analyze broad patterns in order to know where to go next.
...
ANSWER: Points scored seems to have the greatest impact on wins. Yards gained seems to be the least impactful, shown by the size of the circle on the chart.

## Using the Model

Without using the words "multiple regression model", describe how you could use this model to cover a team or game.

This model allows you to analyze the exact root for the success of a team. Inversely, you can find a typically positive stat and see that it might not be aiding a team as much. If a team is gaining a lot of yards but not scoring, you are able to point that out.
