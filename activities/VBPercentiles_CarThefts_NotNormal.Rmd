...
**Your answer**: It's easier to assume data is normal, the challenge that arises when it doesn't meet that assumption is that the data isn't necessarily objective, slanted in one direction or another. For example, if you use a nicer part of a city to represent the entirety, the data would be skewed. An example of a story a reporter might write about data that isn't normally distributed comes from income and housing prices in regards to different areas in a city. 
...
**Answer**: Maryland is in the top 20 for aces and is in the 95th percentile.
...
**Answer**: The distribution is skewed to the right as most of the values are clustered around the mean. The range of data covered by one standard deviation in either direction consists of most of the data, the main set of values. 
