...
```{r}
gdp |> summarize(correlation = cor(GDP, DJIA))
```
...
gdp |> 
  ...
  labs(title = "Correlation between Dow Jones Industrial Average and Gross Domestic Product.")
```
...
ANSWER: There is a strong, positive correlation between DJIA and GDP, shown by the scatterplot. As a country produces product, the industrial average rises.
