...
spotify |> 
  ...
  geom_histogram(aes(x = danceability), binwidth = 0.05) +
  geom_vline(aes(xintercept = mean(danceability, na.rm = TRUE)), color = "red", linetype = "dashed", size = 1) +
  geom_vline(aes(xintercept = mean(danceability, na.rm = TRUE) - sd(danceability, na.rm = TRUE)), color = "blue", linetype = "dashed", size = 1) +
  geom_vline(aes(xintercept = mean(danceability, na.rm = TRUE) + sd(danceability, na.rm = TRUE)), color = "blue", linetype = "dashed", size = 1)
...
Dancablility seems skewed to the right, which makes sense since music is made to keep rhythm. Most of the values reside within one standard deviation of the mean, with some outliers.

Story pitch: We can infer from the data that dancable music is popular. Using this stat, we can measure the artists with the most songs that fit this and further analyze what about their music makes it dancable. Is it intentionally designed that way or does the artist just have a good sense for things. 
